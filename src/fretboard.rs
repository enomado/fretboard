...
impl Fretboard {
    pub fn fret_pos(&self, n: Fret) -> f32 {
        match self.config {
            FretConfig::Log => fret_position_log_range(&self.screen_size_x, &self.fret_range_show, n),
            FretConfig::Linear => fret_position_linear_range(&self.screen_size_x, &self.fret_range_show, n),
        }
    }
