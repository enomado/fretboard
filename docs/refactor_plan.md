...
| Фаза | Суть | Статус |
|---|---|---|
| Ф1 | Отравленный мьютекс = паника, а не тихий дефолт | ⬜ |
| Ф2 | Один цикл аудио-воркера вместо двух копий | ⬜ |
| Ф3 | `core_types` не зависит от UI (`Scale` → `Mark`) | ⬜ |
...
**lib 185 passed / 15 ignored, `tests/pill_layout.rs` 2 passed**, бинарники 0 тестов. Тесты гонять в `--release`: в debug кадр анализа съедает
