...
флейкает~~ — **починено 2026-09-23** (часы `Rig` = число поданных сэмплов, см. «Попутные
находки»); падение теперь — настоящая регрессия, перегонять нечего. После попутных находок:
**lib 196**. Тесты гонять в `--release`: в debug кадр анализа съедает
...
- ✅ **Починено** («test(audio): часы Rig по сэмплам — флейк release_ghosts снят»): пайплайны
  получили `push_samples_at(now, …)` (`push_samples` = он же с `Instant::now()`, продакшн не
  тронут), `Rig::feed` штампует чанк моментом его последнего сэмпла и не спит. **Вскрылось
  главное:** на идеальных часах призрака нет — старый `Rig` попадал в него случайно. Замер
  (чанки 10 мс): `update_ms` 8/10 → призрак MIDI 24; 16/20/30/40/60/80 → нет. Со `sleep`
  накладные расходы чанка обычно выталкивали его за 16-мс гейт ⇒ банк публиковал каждые
  10 мс ⇒ призрак; под нагрузкой — как повезёт. Тест теперь явно ставит `update_ms = 10`; без
  этой строки он падает детерминированно. Новая находка — ниже («призрак зависит от каденции»).
  Исходная запись: **Флейк `audio::core::tests::release_ghosts_are_written_after_a_note`** (найден на
...
- ✅ **Сделано** («docs(rustdoc): ноль битых intra-doc ссылок на всех трёх целях»): к 09-23 их
  ...
  и экранирование `\[0,1\]`.
- 🔴 **Призрак после ноты зависит от каденции банка — пользовательского ползунка** (найдено
  2026-09-23 при починке флейка выше, `audio/core.rs`, тест `release_ghosts_…`): при
  `ResonatorSettings::update_ms` ≤ 10 мс мгновенный обрыв A4 пишет призрак (MIDI 24), при ≥ 16 —
  нет. Каденция — ползунок 8..80 мс, и решать, будет ли записана лишняя нота, ему нельзя (то же
  правило, что `memory/display_settings_must_not_steer_the_detector.md`; Витерби ради этого уже
  сделан dt-инвариантным, `dsp::trellis`). Подозреваемый — ремонтный слой, считающий в **кадрах**:
  медиана `OctaveGate` (`octave_gate.rs:43`, `MEDIAN_WINDOW = 5` = 50 мс при 10 мс и 100 мс при
  20 мс — ровно «~50 мс» из описания бага в тесте) и `LEAP_CONFIRM_FRAMES` (`melody.rs:372`). Не
  проверено; оракул готов — тот же тест с `update_ms` 10 vs 20. Сам призрак (поздний гейт
  тишины на сглаженном уровне метра + банк, который не умеет молчать) — по-прежнему вопрос к
  инструменту, см. doc теста.
- ✅ (снят вместе с флейком выше) **Флейк `release_ghosts_…` под нагрузкой чаще, чем 1 из 5** (Ф7, 2026-09-22): при load
