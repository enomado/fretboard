...
3. ~~Следующая — Ф7~~ — **✅ сделана в тот же день** («refactor(audio): pub mod types вместо
   23 реэкспортов»). **План исполнен целиком.** Базовый счётчик после Ф7: lib **195 passed /
   15 ignored**. Дальше — только пункты «Ждут решения хозяина» ниже и «Попутные находки» плана.
