...
2. Следующая фаза — **Ф6в** (целые MIDI-ноты → `PNote`, единый `BankRange`). Ниже —
   разведка по ней: три посылки плана уже сверены с кодом, и две из них устарели.
3. После Ф6в — **Ф7** (снять 22 `pub use` из `audio/mod.rs`). Она последняя по замыслу.
