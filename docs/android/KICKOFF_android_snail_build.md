...
он **не Windows-специфичный**. Артефакты: `<scratchpad>/jni-test/` (одноразовый,
в репо не тащим).

### NEXT SESSION
- **Ответить в #833** (черновик готовился в сессии 07-25, НЕ опубликован — решение юзера):
  (1) кросс-платформенное подтверждение репро с цепочкой из `hs_err` выше;
  (2) главное для нас — подпереть «single sustained .so ⇒ Android/NativeActivity вне зоны
  риска» ФАКТАМИ вместо «Copilot tells me» (у автора это единственное обоснование и в #526,
  и в #833): `System.loadLibrary` из NativeActivity держит библиотеку класслоадером
  приложения, живущим до смерти процесса; ART выгружает `.so` только вместе с
  класслоадером; ⇒ для нашего юзкейса недостижимо;
  (3) `JNI_Unload`-хук (автор сам зовёт «probably bad») и вариант «документировать контракт
  вместо кода».
- Мейнтейнеров jni-rs в #833 пока НЕТ: единственный комментарий — его же `cc:
  @ColonelThirtyTwo` (24.07). Тред фактически пустой ⇒ наш ответ = первое внешнее
  подтверждение, а не «влезание в чужой спор».
- **Docs-PR в android-activity** («Runtime permissions» в README) — по-прежнему открыт и
...
- **Рантайм-тест смёрженного `PermissionRequestLifecycle`** на Redmi/MIUI — нужен
  `cargo-apk2` (cargo-apk наш не умеет `classes.dex`, см. ниже).
